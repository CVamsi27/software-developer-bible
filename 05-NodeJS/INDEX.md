# Node.js — Index

> **5 files** — Event loop internals, streams, buffers, clustering, and interview questions.

[![Files](https://img.shields.io/badge/files-5-blue)](INDEX.md)
[![Category](https://img.shields.io/badge/category-Backend-success)](.)
[![Status](https://img.shields.io/badge/status-complete-brightgreen)](.)

| # | File | Topics |
|---|------|--------|
| 01 | [Event Loop](01-Event-Loop.md) | Phases, timers, I/O callbacks, setImmediate, nextTick |
| 02 | [Streams](02-Streams.md) | Readable, writable, transform, backpressure, pipeline |
| 03 | [Buffers](03-Buffers.md) | Buffer creation, encoding, allocation, TypedArray |
| 04 | [Clustering](04-Clustering.md) | Cluster module, worker processes, IPC, load balancing |
| 05 | [Interview Questions](05-Interview-Questions.md) | 50+ curated questions with answers |

---

**Cross-references:** [JavaScript Event Loop](../01-JavaScript/08-Event-Loop.md) | [NestJS](../06-NestJS/) | [Docker](../13-Docker/)
---

## Navigation

[← Previous: Next.js](../04-NextJS/INDEX.md) · [🏠 Back to Index](../INDEX.md) · [Next: NestJS →](../06-NestJS/INDEX.md)

...
> **TL;DR:** Pagination is the API’s mechanism for returning large result sets in bounded responses. Offset pagination is simple but breaks under writes; cursor pagination is stable but doesn’t support random page jumps — know when to use which, and always return total/next/prev metadata.
>
